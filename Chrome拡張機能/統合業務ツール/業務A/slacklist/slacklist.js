...
(function() {
    ...
    function initializeSlackAdder() {
        ...
        try {
            ...
            if (urlParams.get('modal') === 'true') {
                ...
                // ヘッダーが消えた分、メインコンテンツの余白を調整する場合
                const containerGroup = document.querySelector('.container-group');
                if (containerGroup) {
                     // common.css で .header の高さ分だけ .container-group に margin-top が設定されている場合、
                     // それを打ち消す (例: common.css が margin-top: 60px を設定している場合)
                     // containerGroup.style.marginTop = '0px'; 
                     
                     // もし common.css が body に padding-top を設定している場合
    ...
    async function handleAddToSlack() {
        ...
        console.log('=== Slack API リクエスト ===');
        console.log('送信するペイロード:', JSON.stringify(payload, null, 2));
        ...
        try {
            ...
            console.log('レスポンス全体:', JSON.stringify(result, null, 2));

            if (result.ok) {
                console.log('✅ 成功!');
                
                if (result.item && result.item.fields) {
                    console.log('作成されたフィールド:', result.item.fields);
                }
                ...
                console.error('=== Slack API エラー ===');
                console.error('エラーコード:', result.error);
...
*依頼者:* ${requestorName}`;
        ...
        console.log('=== Slack通知 送信 ===');
        console.log('送信先チャンネル:', CHANNEL_ID);
